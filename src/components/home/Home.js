...
function Home() {
  ...
  useEffect(() => {
    if (checkOwner.match('daniadarweesh@gmail.com')){
      setShow(true);
    }
    setAllPosts(() => {
  ...
  function savePost(e) {
    ...
    const currenDate = Helper.getCurrentDate();
      ...
      localStorage.setItem('blog-owner', JSON.stringify({
       name: 'dania',
       email: 'daniadarweesh@gmail.com',
       password: 'Dania2022$',
       posts: [{
         id: Math.ceil(Math.random() * 1000),
        text: "Who's better? Semo or Memo ?",
         date:currenDate,
         likes: 5,
         disLikes: 0,
        comments: []
  ...
  return (
    <>
      ...
      <form className='home__addPostForm' onSubmit={savePost}>
        <input type="text" onChange={(e) => setPost(e.target.value)} placeholder="write new post Dandoon" value={post} />
